...
test("연수담 첫 화면을 서버에서 정상 렌더링한다", async () => {
  ...
  assert.match(html, /전남교육연수포털 일괄 과정/);
  assert.match(html, /href="https:\/\/www\.jnstudy\.kr\/"/);
...
test("계정 API가 D1 저장·수정 충돌·삭제를 안전하게 처리한다", async () => {
  ...
  const previousVersion = await mf.dispatchFetch(
    "http://localhost/api/training-state",
    {
      method: "PUT",
  ...
  assert.equal(previousVersion.status, 422);
